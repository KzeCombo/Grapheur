...
public class MainFenetre extends JFrame {
	...
	JLabel x,y,fx,xmin,xmax,deltax, pas, ymin,ymax,deltay,exp;
	JTextField  tx,ty,tfx,txmin,txmax, tpas, tdeltax,tymin,tymax,tdeltay,texp;
	JButton button, buttonOk, zoomIn, zoomOut;
	Canvas mycanvas ;
	Fonction fonc;
    private double[] valeurParametres = { -1.0, +1.0, 0.2, -1.0, +1.0, 0.2, 0.2 };
	...
	public MainFenetre(){
		...
		row.setLayout(new GridLayout(7,2));
		...
		// Les bouton Avant / Arrière
		JPanel zoom = new JPanel();
		zoom.setLayout(new GridLayout(1,2));
		...
		west.add(zoom, BorderLayout.SOUTH);
		...
		// listerner pour le deuxième button "Tracer"
	...
	private void preparerLaCourbe() throws Exception{
		try{
			...
			valeurParametres[6] = Double.parseDouble(tpas.getText());
